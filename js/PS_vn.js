...
Queensland dengue outbreak	2009	2009	1	Queensland, Australia	Dengue fever	Australia	Virus	Epidemic`;
    ...
    data.forEach(d => {
        ...
        if (deaths < 1000) {
            ...
            d.radius = 10;
        } else if (deaths < 1000000) {
            d.radius = 15;
        } else if (deaths < 10000000) {
            d.radius = 22;
        } else if (deaths < 20000000) {
            d.radius = 31;
        } else if (deaths < 30000000) {
            d.radius = 42;
        } else if (deaths < 50000000) {
            d.radius = 55;
        } else {
            d.radius = 70;
        }
    ...
    const continentColors = {
        ...
        "Worldwide": "#c03030ff",
        "All Continents": "#FFFFFF"
    };

    function getContinentColor(c) {
        if (c.includes("Africa")) return continentColors["Africa"];
        if (c.includes("Asia")) return continentColors["Asia"];
        if (c.includes("Europe")) return continentColors["Europe"];
        if (c.includes("North America")) return continentColors["North America"];
        if (c.includes("South America")) return continentColors["South America"];
        if (c.includes("Worldwide") || c.includes("All Continents") || c.includes("Global")) return continentColors["Worldwide"];
    ...
    function draw(dataSet) {
        ...
        let groups = gPoppies.selectAll(".poppy-group")
            ...
            .on("mouseover", function (d) {
                ...
                $(".duration .content").text(d.duration === 0.5 ? "< 1 năm" : Math.round((d.end - d.start)) + " năm" + " (" + d.start + "-" + d.end + ")");
                $(".fatalities .content").text((d.deaths_M * 1000000).toLocaleString());
                $(".cause-row .content").text(labels[d.cause] || d.cause);
            })
        ...
        groups.each(function (d) {
            ...
            let auraColor = getContinentColor(d.continent);
            let centerColor = auraColor;
            ...
            // Fewer and smaller white dots to not block the continent color
            for (let i = 1; i <= 6; i++) {
                let a = 2 * Math.PI * (i - 1) / 6;
                g.append("circle")
                    .attr("cx", Math.cos(a) * r * 0.40).attr("cy", Math.sin(a) * r * 0.40)
                    .attr("r", r * 0.12)
                    .style("fill", "white")
                    .style("opacity", 0.60);
            }
